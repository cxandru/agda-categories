...
liftTerminal2 : Terminal (F-Coalgebras F) → Terminal (F-Coalgebras (LiftAlgebras T F μ))
liftTerminal2 νF = record
  { ⊤ = AlgebrasT̂⇒CoalgebrasF̂ .F₀ $ ⊤
  ; ⊤-is-terminal = record
    { ! = λ {A = X} → let
      t-morph = IsTerminal.! ⊤-is-terminal { A = CoalgebrasF̂⇒AlgebrasT̂ .F₀ X }
      in record
      { f = record { f = F-Coalgebra-Morphism.f
                           (F-Algebra-Morphism.f t-morph) ; commutes = F-Algebra-Morphism.commutes t-morph }
      ; commutes = F-Coalgebra-Morphism.commutes
                     (F-Algebra-Morphism.f t-morph)
      }
    ; !-unique = λ {A = X} g → IsTerminal.!-unique ⊤-is-terminal {A = CoalgebrasF̂⇒AlgebrasT̂ .F₀ X}
        record
        { f = record
  ...
  where
    ...
    open IsTerminal ⊤-is-terminal
    open Functor

-- F-Coalgebra-Morphism.f (F-Algebra-Morphism.f (IsTerminal.! ⊤-is-terminal))
-- F-Algebra-Morphism.commutes (IsTerminal.! ⊤-is-terminal)
-- !-unique (F-Algebra-Morphism.f g)
